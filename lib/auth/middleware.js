...
// Public marketing routes on the apex/www host: fully static, no session needed.
// Skipping the Supabase getUser() round-trip here cuts TTFB on exactly the pages
// that have to rank. Clinic subdomains still get the full auth + routing pass.
...
export async function updateSession(request) {
  ...
  const hostSub = getSubdomain(host);
  if ((!hostSub || RESERVED.has(hostSub)) && MARKETING_PATHS.has(request.nextUrl.pathname)) {
    return response;
  }
