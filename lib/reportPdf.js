...
// ── Text safety for jsPDF's built-in fonts ──────────────────────────────────
// jsPDF's standard Helvetica/Times/Courier use WinAnsi (cp1252) encoding. Any
// character outside that set is emitted as its raw two-byte codepoint, which
// produces BOTH garbled glyphs (≥ becomes ") and a phantom space between every
// remaining letter in the string (F l a g g e d   a t). Everything written to
// the PDF goes through here first.
//
// Comparison operators are converted to words rather than >= / <= because these
// documents get read by surveyors and billing auditors, and "at or above 40%"
// is unambiguous in a way a math operator isn't.
function pdfSafe(v) {
  if (v == null) return "";
  let s = String(v)
    .replace(/\bat\s*[≥⩾]\s*/gi, "at or above ")
    .replace(/\bat\s*[≤⩽]\s*/gi, "at or below ")
    .replace(/[≥⩾]\s*/g, ">= ")
    .replace(/[≤⩽]\s*/g, "<= ")
    .replace(/[≈∼]/g, "~")
    .replace(/[≠]/g, "!=")
    ...
    .replace(/[…]/g, "...")
    .replace(/\u00A0/g, " ")
    // Anything still outside cp1252 gets dropped rather than allowed to corrupt
    // the rest of the line. En dash, em dash, euro and trademark are explicitly
    // kept — they live above U+00FF but WinAnsi does encode them.
    .replace(/[^\x00-\xFF\u2013\u2014\u20AC\u2122]/g, "");
  return s.replace(/\s{2,}/g, " ").trim();
}

// Sanitize a 2D table body/head in place-safe fashion.
function safeRows(rows) {
  if (!Array.isArray(rows)) return rows;
  return rows.map((r) => (Array.isArray(r) ? r.map(pdfSafe) : r));
}
...
function heading(doc, y, label, sub) {
  ...
  doc.text(pdfSafe(label), M.left, y);
  y += 4.5;
  if (sub) {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.2);
    doc.setTextColor(...SLATE);
    const lines = doc.splitTextToSize(pdfSafe(sub), CONTENT_W);
...
function calloutRow(doc, y, items) {
  ...
  items.forEach((it, i) => {
    ...
    doc.text(pdfSafe(it.label).toUpperCase(), x + 4, y + 6);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(15);
    doc.setTextColor(...(it.tone || INK));
    doc.text(pdfSafe(it.value), x + 4, y + 14.5);
    if (it.note) {
      doc.setFont("helvetica", "normal");
      doc.setFontSize(6.8);
      doc.setTextColor(...SLATE);
      doc.text(doc.splitTextToSize(pdfSafe(it.note), w - 8)[0] || "", x + 4, y + 19);
    }
  });
  return y + h + 6;
}
...
// Single funnel for every table in the report. Sanitizing here (rather than at
// each call site) means content is cleaned BEFORE autoTable measures it, so line
// wrapping and column widths are computed against what actually gets drawn.
function runTable(doc, y, config) {
  const cfg = { ...tableBase(), startY: y, ...config };
  cfg.head = safeRows(cfg.head);
  cfg.body = safeRows(cfg.body);
  if (cfg.rowPageBreak == null) cfg.rowPageBreak = "avoid";
  doc.autoTable(cfg);
...
function drawCover(doc, meta, markData) {
  ...
  doc.setFontSize(30);
  doc.setTextColor(...INK);
  doc.text(doc.splitTextToSize(pdfSafe(meta.docTitle), CONTENT_W), M.left, 92);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(15);
  doc.setTextColor(...NAVY);
  const nameLines = doc.splitTextToSize(pdfSafe(meta.clinicName), CONTENT_W);
  ...
  const scopeLines = doc.splitTextToSize(pdfSafe(scopeBody), CONTENT_W - 12);
...
function decoratePages(doc, meta, markData) {
  ...
  for (let p = 1; p <= total; p++) {
    ...
    doc.setFontSize(6.6);
    doc.setTextColor(...MUTE);
    const left = pdfSafe(`${meta.clinicName}${meta.ccn ? ` · CCN ${meta.ccn}` : ""}`);
...
function sectionSSVI(doc, y, ctx) {
  ...
  // Widths total CONTENT_W (179.9mm). The threshold and remedy columns carry the
  // longest prose, so they get the room; #, Result and Pts are fixed and narrow.
  const colStyles = mode === "full"
    ? { 0: { cellWidth: 8, halign: "center", textColor: MUTE }, 1: { cellWidth: 32, fontStyle: "bold" }, 2: { cellWidth: 44, fontSize: 7.6, textColor: SLATE }, 3: { cellWidth: 19, halign: "center", fontSize: 7.6 }, 4: { cellWidth: 8, halign: "center", fontStyle: "bold" }, 5: { fontSize: 7.6, textColor: SLATE } }
    : { 0: { cellWidth: 8, halign: "center", textColor: MUTE }, 1: { cellWidth: 50, fontStyle: "bold" }, 2: { fontSize: 7.8, textColor: SLATE }, 3: { cellWidth: 24, halign: "center", fontSize: 7.8 }, 4: { cellWidth: 10, halign: "center", fontStyle: "bold" } };
...
function sectionCategories(doc, y, ctx) {
  ...
  cats.forEach((cat) => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(...NAVY);
    doc.text(pdfSafe(dash(cat.label)), M.left, y);
    ...
    if (metaLine) { doc.text(pdfSafe(metaLine), M.left, y); y += 4; }
    ...
    if (actions.length) {
      ...
      doc.setTextColor(...INK);
      actions.forEach((a) => {
        const lines = doc.splitTextToSize(pdfSafe(a), CONTENT_W - 6);
...
export async function generateComplianceReport(opts) {
  ...
  // Last line of defence. Everything is sanitized before it's measured, so this
  // is a no-op in the normal path — it exists so a stray character coming from
  // clinic data or a model response can never garble a whole line of the report.
  const _text = doc.text.bind(doc);
  doc.text = function (txt, x, y, opts, ...rest) {
    if (typeof txt === "string") txt = pdfSafe(txt);
    else if (Array.isArray(txt)) txt = txt.map((t) => (typeof t === "string" ? pdfSafe(t) : t));
    return _text(txt, x, y, opts, ...rest);
  };
  doc.setProperties({
